...
test("personal account can be created and then signed in", async ({ page }) => {
  ...
  await expect(
    page.getByText("Your personal account is ready. Sign in below."),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: "Welcome to SkulGo" }),
  ).toBeVisible();
  await expect(page.getByText(email)).toBeVisible();
  await expect(
    page.getByRole("link", { name: /Find a school/ }),
  ).toBeVisible();
});
...
test("school owner sees their personal account and can open the owner dashboard", async ({
  page,
}) => {
  ...
  await page
    .getByLabel("Organization / registered business name")
  ...
  await page
    .getByLabel("Password (minimum 12 characters)")
    .fill(password);
