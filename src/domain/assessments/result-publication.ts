...
export async function publishAssessmentResult(schoolId: string, assessmentId: string, publishedByUserId: string) {
  ...
  const existing = await db.auditEvent.findFirst({ where: { schoolId, entityType: "AssessmentDefinition", entityId: assessmentId, action: "assessment.result_published" }, select: { id: true } });
  ...
  const event = await db.auditEvent.create({
    data: {
      ...
      previousState: { status: "APPROVED", approvedAt: approval.occurredAt.toISOString() },
