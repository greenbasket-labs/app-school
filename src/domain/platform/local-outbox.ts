...
export async function getLocalRecord<T>(id: string): Promise<LocalRecord<T> | null> {
  ...
  try {
    ...
    return (await requestResult(transaction.objectStore(LOCAL_STORES.records).get(id))) as LocalRecord<T> | undefined ?? null;
  } finally {
    db.close();
  }
}
...
export async function getPendingOutbox(schoolId: string): Promise<LocalOutboxItem[]> {
  const db = await openAppSchoolLocalDb();
  try {
    const transaction = db.transaction(LOCAL_STORES.outbox, "readonly");
    const index = transaction.objectStore(LOCAL_STORES.outbox).index("schoolStatus");
    const rows = await requestResult(index.getAll(IDBKeyRange.bound([schoolId, "PENDING"], [schoolId, "SYNCING"])));
    return rows.filter((row) => row.status === "PENDING") as LocalOutboxItem[];
  } finally {
    db.close();
  }
}
...
export async function updateOutboxStatus(
  ...
  details: { attemptCount?: number; lastError?: string | null } = {},
) {
  const db = await openAppSchoolLocalDb();
  try {
    const transaction = db.transaction(LOCAL_STORES.outbox, "readwrite");
    const store = transaction.objectStore(LOCAL_STORES.outbox);
    const existing = (await requestResult(store.get(operationId))) as LocalOutboxItem | undefined;
    if (!existing) return;
    store.put({
      ...existing,
      status,
...
export function syncStateFromOutboxStatus(status: LocalOutboxItem["status"]): LocalSyncState {
  switch (status) {
    case "PENDING":
      return "PENDING_SYNC";
    case "SYNCING":
