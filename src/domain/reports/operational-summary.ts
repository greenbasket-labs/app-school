...
export async function getOperationalSummary(schoolId: string) {
  const [students, staff, todayAttendance, openInvoices, outstanding] = await Promise.all([
    ...
    db.$queryRaw<Array<{ count: bigint }>>`SELECT COUNT(*)::bigint AS count FROM "StudentFeeInvoice" i WHERE i."schoolId" = ${schoolId} AND i."status" = 'OPEN' AND i."amount" > COALESCE((SELECT SUM(p."amount") FROM "PaymentRecord" p WHERE p."invoiceId" = i."id"), 0)`,
