...
const inputSchema = z.object({
  email: z.string().email(), password: z.string().min(12).max(128), organizationName: z.string().min(2).max(200), schoolName: z.string().min(2).max(200), cacNumber: z.string().min(4).max(64).optional().or(z.literal("")),
});
...
export async function registerSchoolOwner(raw: RegisterSchoolOwnerInput) {
  ...
  const normalizedCacNumber = input.cacNumber ? normalizeCacNumber(input.cacNumber) : null;
  ...
  try {
    ...
    return await db.$transaction(async (tx) => {
      ...
      const organization = await tx.organization.create({ data: { name: input.organizationName.trim(), normalizedName: normalizeOrganizationName(input.organizationName), ...(normalizedCacNumber ? { identity: { create: { cacNumber: input.cacNumber!.trim(), normalizedCacNumber } } } : {}) }, select: { id: true, createdAt: true } });
