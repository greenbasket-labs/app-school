...
export default function ScoreCaptureWorkspace({ schoolId, assessments, initialAssessmentId }: { schoolId: string; assessments: Assessment[]; initialAssessmentId: string }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true); setMessage(""); setSubmitted(false);
    fetch(`/api/schools/${schoolId}/assessments/scores?assessmentId=${encodeURIComponent(assessmentId)}`)
      .then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.message ?? "Could not load the class roster."); return data as RosterResponse; })
      .then((data) => { if (cancelled) return; setStudents(data.students); setMaxScore(data.assessment.maxScore); setDrafts(Object.fromEntries(data.students.map((student) => [student.studentId, student.score == null ? "" : String(student.score)]))); })
  ...
  async function saveScore(studentId: string) {
    ...
    try {
      const response = await fetch(`/api/schools/${schoolId}/assessments/scores`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ assessmentId, studentId, score }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.message ?? "Could not save score.");
      setStudents((current) => current.map((student) => student.studentId === studentId ? { ...student, score: data.score.score } : student)); setMessage("Score saved and audited.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Could not save score."); }
  ...
  return (
    <div style={{ marginTop: 24, display: "grid", gap: 20 }}>
      ...
      <section style={{ border: "1px solid #dce3df", borderRadius: 16, padding: 20 }}>
        {loading ? <p>Loading class roster…</p> : students.length === 0 ? <p style={{ color: "#53615a" }}>No active enrolled students found for this assessment.</p> : <div style={{ display: "grid", gap: 10 }}>{students.map((student, index) => <div key={student.studentId} style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 120px auto", gap: 10, alignItems: "end", borderBottom: index === students.length - 1 ? 0 : "1px solid #edf0ee", paddingBottom: index === students.length - 1 ? 0 : 10 }}><div><strong>{student.lastName} {student.firstName} {student.middleName ?? ""}</strong><div style={{ color: "#53615a", fontSize: 12 }}>{student.admissionNumber}</div></div><input disabled={submitted} value={drafts[student.studentId] ?? ""} onChange={(event) => setDrafts((current) => ({ ...current, [student.studentId]: event.target.value }))} inputMode="decimal" type="number" min="0" max={maxScore} step="0.01" placeholder={`0–${maxScore}`} style={inputStyle} aria-label={`Score for ${student.firstName} ${student.lastName}`} /><button onClick={() => saveScore(student.studentId)} disabled={submitted || savingStudentId === student.studentId} style={buttonStyle}>{savingStudentId === student.studentId ? "Saving…" : "Save"}</button></div>)}</div>}
