...
import StudentFeeAssignmentWorkspace from "./workspace";

export default async function StudentFeeAssignmentsPage({ params }: { params: Promise<{ schoolId: string }> }) {
    ...
    <main style={{ minHeight: "100vh", padding: 24 }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ marginTop: 20 }}>
          <p style={{ margin: 0, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", fontSize: 13 }}>Student fee assignments</p>
          <h1 style={{ margin: "8px 0 6px", fontSize: 34 }}>{membership.school.name}</h1>
          <p style={{ margin: 0, color: "#53615a" }}>Connect an existing fee to a specific enrolled student. This records the expected fee amount; payments come later.</p>
        </div>
        <StudentFeeAssignmentWorkspace schoolId={schoolId} />
      </div>
    </main>
  );
}
