...
export default async function SchoolWorkspacePage({ params }: { params: Promise<{ schoolId: string }> }) {
  ...
  const membership = await db.membership.findFirst({
    ...
    select: { id: true, school: { select: { id: true, name: true, status: true, setupStatus: true } }, capabilities: { select: { capability: { select: { code: true } } } } },
  });
  ...
  return (
    <main style={{ minHeight: "100vh", padding: 32 }}><div style={{ maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{ marginTop: 24, background: "white", borderRadius: 20, padding: 32, boxShadow: "0 8px 28px rgba(0,0,0,.05)" }}>
        ...
        {canManageSchool && membership.school.setupStatus !== "COMPLETED" && <div style={{ marginTop: 16, padding: 20, borderRadius: 14, background: "#f8faf9" }}><strong>Next: school setup</strong><p style={{ marginBottom: 0, color: "#53615a" }}>Capture the minimum operational configuration needed to make the school usable.</p></div>}
      </div>
    </div></main>
  );
}
