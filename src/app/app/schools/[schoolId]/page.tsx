    ...
    <main style={{ minHeight: "100vh", padding: 32 }}>
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div style={{ marginTop: 24, background: "white", borderRadius: 20, padding: 32, boxShadow: "0 8px 28px rgba(0,0,0,.05)" }}>
          ...
          <p style={{ color: "#53615a", lineHeight: 1.6 }}>School identity and access are established first. Operational workflows now sit behind the same school boundary.</p>
          ...
          {canViewAttendance && (
            <Link href={`/app/schools/${schoolId}/attendance`} style={{ display: "block", marginTop: 20, padding: 20, borderRadius: 14, background: "#f3f7f4", color: "inherit", textDecoration: "none" }}>
              <strong>Daily attendance →</strong>
              <p style={{ margin: "6px 0 0", color: "#53615a" }}>Load a class roster, mark attendance quickly, and save the day in one action.</p>
            </Link>
          )}

          {canManageSchool && membership.school.setupStatus !== "COMPLETED" && (
            <div style={{ marginTop: 16, padding: 20, borderRadius: 14, background: "#f8faf9" }}><strong>Next: school setup</strong><p style={{ marginBottom: 0, color: "#53615a" }}>Capture the minimum operational configuration needed to make the school usable.</p></div>
          )}
        </div>
      </div>
    </main>
