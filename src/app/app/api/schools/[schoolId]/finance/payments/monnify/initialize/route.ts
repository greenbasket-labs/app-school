...
import { initializeMonnifyPayment, MonnifyPaymentError } from "@/domain/finance/monnify";
...
export async function POST(request: Request, { params }: { params: Promise<{ schoolId: string }> }) {
  try {
    ...
    const payment = await initializeMonnifyPayment(schoolId, parsed.data.invoiceId, parsed.data.payerEmail, parsed.data.payerName);
    return NextResponse.json({ ok: true, payment });
  } catch (error) {
    if (error instanceof MonnifyPaymentError) return NextResponse.json({ ok: false, error: "PAYMENT_FAILED", message: error.message }, { status: 400 });
