...
import {
  AssessmentScoreConflictError,
  AssessmentScoreValidationError,
  getAssessmentScoreRoster,
  saveAssessmentScore,
} from "@/domain/assessments/score-service";
...
export async function GET(
  _request: Request,
  {
    ...
    params: Promise<{ schoolId: string; assessmentId: string }>;
  },
) {
  try {
    ...
    return NextResponse.json({
      ok: true,
      ...(await getAssessmentScoreRoster(schoolId, assessmentId)),
    });
  } catch (error) {
    if (
      error instanceof AuthorizationError ||
      error instanceof ModuleDisabledError ||
      error instanceof AssessmentScoreValidationError
    ) {
      return NextResponse.json(
        {
          ok: false,
        ...
        {
          status:
            error instanceof AssessmentScoreValidationError ? 404 : 403,
        },
      );
    }
...
export async function POST(
  ...
  try {
    ...
    if (error instanceof AssessmentScoreValidationError) {
      return NextResponse.json(
        {
          ok: false,
          error: "INVALID_SCORE",
    ...
    if (error instanceof AssessmentScoreConflictError) {
      return NextResponse.json(
        {
          ok: false,
          error: "SCORE_EXISTS",
