...
import {
  ...
  ResultApprovalValidationError,
} from "@/domain/assessments/result-approval";
...
export async function POST(
  ...
  try {
    ...
    const membership = await requireCapability(
      session.user.id,
      schoolId,
      CAPABILITIES.APPROVE_RESULTS,
    );
    await requireSchoolModule(membership.schoolId, "ASSESSMENTS");

    const input = schema.parse(await request.json());
    const event = await approveAssessmentResult(
      schoolId,
      input.assessmentId,
      session.user.id,
    );
    ...
    if (error instanceof ResultApprovalValidationError) {
      return NextResponse.json(
        { ok: false, error: "APPROVAL_BLOCKED", message: error.message },
        { status: 400 },
      );
    }
