...
export async function POST(request: Request, { params }: { params: Promise<{ schoolId: string }> }) {
  try {
    ...
    const event = await publishAssessmentResult(schoolId, input.assessmentId, session.user.id);
    return NextResponse.json({ ok: true, publication: { id: event.id, assessmentId: input.assessmentId, publishedAt: event.occurredAt } }, { status: 201 });
  } catch (error) {
    ...
    if (error instanceof ResultPublicationValidationError) return NextResponse.json({ ok: false, error: "PUBLICATION_BLOCKED", message: error.message }, { status: 400 });
